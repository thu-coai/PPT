...
def enc_dec_get_params_for_adapter_optimization(module: nn.Module):
    params = [{'params': []}]

    for t in module.named_parameters():
        if "adapter" not in t[0]:
            t[1].requires_grad_(False)
    ...
    for t in module.named_modules():
        if "adapter" in t[0]:
            if torch.distributed.get_rank() == 0:
                print("Update params", t[0])
            params[0]['params'].extend(
                [p for p in list(t[1]._parameters.values()) if p is not None])
