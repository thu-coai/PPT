...
from data_utils import make_builder
...
class Encoder(object):
    ...
    def encode(self, all_lines):
        ...
        for i, sent_ids in enumerate(doc_ids):
            ...
            pair = sent_ids[:stat_map[n][0]] + [58]
            for i, option in enumerate(options):
                pair.extend([number_map[i], 5] + option + [5])
            pair += [58] + self.tokenizer.encode("The correct one is ") + [Encoder.tokenizer.get_sentinel_id(0)]

            label_ids = [0, Encoder.tokenizer.get_sentinel_id(0)] + [number_map[label]] + [Encoder.tokenizer.get_sentinel_id(1)]
...
def get_args():
    ...
    group.add_argument('--input_neg1', type=str, help='Path to input TXT')
    group.add_argument('--input_neg2', type=str, help='Path to input TXT')
    group.add_argument('--input_neg3', type=str, help='Path to input TXT')
    group.add_argument('--input_neg4', type=str, help='Path to input TXT')

    group = parser.add_argument_group(title='tokenizer')
    group.add_argument('--tokenizer_path', default="../../vocab_en", type=str, help='Path of tokenizer')

    group = parser.add_argument_group(title='output data')
    group.add_argument("--output_path", default="../../pretrain_data/preprocessed", type=str)
    group.add_argument('--output_prefix', default="nss_uni", type=str,
...
def main():
    ...
    builder_context = make_builder(context_bin_file, impl=args.dataset_impl, dtype=np.uint16)
    builder_target = make_builder(target_bin_file, impl=args.dataset_impl, dtype=np.uint16)
