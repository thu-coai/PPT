...
class Encoder(object):
    ...
    def encode(self, line_pair):
        ...
        # data2
        data2 = line2.strip()
        data2 = data2.split("<@x(x!>")

        if min([len(x) for x in data2]) < 5:
            return None, None, 0
        ...
        if min([len(x) for x in doc_ids2]) < 5:
            return None, None, 0
        ...
        for i, sent_ids in enumerate(doc_ids):
            ...
            pairs.append(pair)
            labels.append([0, Encoder.tokenizer.get_sentinel_id(0)] + [label] + [Encoder.tokenizer.get_sentinel_id(1)])
...
def get_args():
    ...
    group.add_argument('--input2', type=str, help='Path to input TXT')
    ...
    group.add_argument('--output_prefix', default="nsp", type=str,
